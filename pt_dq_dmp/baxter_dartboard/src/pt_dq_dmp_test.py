...
def gen_data(n, off=np.array([0.50, 0.10, 0.10])):
    ...
    tr_r = quat.as_float_array(quat.from_euler_angles(x - .5 * np.pi, x - ((4 * z) / np.pi), x))

    dqtr = np.empty(tr_p.shape[0], dtype=DualQuaternion)
    for i, (p, r) in enumerate(zip(tr_p, tr_r)):
        dqtr[i] = DualQuaternion.from_quat_pose_array(np.append(r, p))
...
def main():
    ...
    # Generate MJT data
    t, dqtr, p, r = gen_data(300)

    # DMP Training
    n = 100
    alpha_y = 12
    dmp_obj = PTDQMP(n, alpha_y)
    ...
    dq_, tw_ = dmp_obj.fit_model(t_, dq0, tw0, dqg, tau=tau)
    r_, p_ = dmp_obj.pose_from_dq(dq_)
