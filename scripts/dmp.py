...
class DMP(object):
    ...
    def fit_step(self, t, y, dy, yg, y0, tau=1, dyg=None):
        ...
        if dyg is not None:
            ...
            # h_ = quat.rotate_vectors(qres, g_)
            # plt.figure()
            # ax = plt.axes(projection='3d')
            # ax.quiver(0, 0, 0, c_[0], c_[1], c_[2], length=.1, color='k')
            # ax.quiver(0, 0, 0, a_[0], a_[1], a_[2], length=.1, color='r')
            # ax.quiver(0, 0, 0, e_[0], e_[1], e_[2], length=.1, color='g')
            # ax.quiver(0, 0, 0, b_[0], b_[1], b_[2], length=.1, color='m')
            # ax.quiver(0, 0, 0, f_[0], f_[1], f_[2], length=.1, color='c')
            # ax.quiver(0, 0, 0, g_[0], g_[1], g_[2], length=.05, color='r')
            # ax.quiver(0, 0, 0, h_[0], h_[1], h_[2], length=.05, color='g')
            ...
            # plt.show()
            # qrot = self.__quat_from_vecs(a_, b_)
            fn = quat.rotate_vectors(qres * qrot, fn)
